...
public class 二七八_第一个错误的版本 {
    public static void main(String[] args) {
        int badVersion = firstBadVersion(9);
        System.out.println("错误的版本是第"+badVersion+"个版本");
    }

    //判断错误版本的接口方法【设置错误版本为第三个版本】
    public static boolean isBadVersion(int x){
        if (x  >= 3) {  //第三个版本之后的都是错误版本
            System.out.println("这是个错误的版本");
            return true;
        } else{
            System.out.println("这是个正确的版本");
            return false;
        }
    }
    ...
    public static int firstBadVersion(int n){
        ...
        while (left < right){ //循环至左右端点都相同
            int pivot = left + (right - left)/2;  //计算中间值（并防止计算时溢出）
            if (isBadVersion(pivot)){
                right = pivot;    //如果pivot是错误版本，那么答案在【left，pivot】中
            }else {
                left = pivot + 1;//如果pivot是正确版本，那么答案在【mid+1，pivot】中 （这里+1是因为我们找的是错误版本）
            }
        }
        //此时left = right = pivot，区间缩为一个点，即为答案
        return left;
    }
}
